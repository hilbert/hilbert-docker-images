...
if [ ! -f $HOME/.mozilla/$F/*.kiosk/prefs.js ]; then 

  $F -CreateProfile kiosk & 
  PID=$!

  while : ; do
    sleep 0.5
    [ -f $HOME/.mozilla/$F/*.kiosk/prefs.js ] && break
...
ls -la $HOME/.mozilla/$F/*.kiosk/prefs.js
ls -la $HOME/.mozilla/$F/*.kiosk/user.js
rm -r  $HOME/.mozilla/$F/*.kiosk/sessionstore-backups

##cat <<EOF | tee -a $HOME/.mozilla/$F/*.kiosk/prefs.js
